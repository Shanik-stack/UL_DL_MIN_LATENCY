...
Start at `experiment.py::main`. Training continues in
`train_precoder_network.py::train_blocklength_aware_precoder_net`. Each epoch
calls `build_training_rollouts.py::_generate_rollout_queries_for_downlink` to discover current
channel/blocklength states, then uses `precoder_network.py` for differentiable
joint model evaluation. Start testing at
`evaluate_precoder_network.py::evaluate_downlink_precoder_net`. `experiment.py` coordinates data,
checkpoints, result directories, and train/test execution.
