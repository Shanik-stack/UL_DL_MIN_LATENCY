"""Run downlink Monte Carlo training and held-out evaluation.
...
from ...objectives.settings import get_convergence_objective_name
...
from latency_optimization.results.metrics import format_optional_db as _format_optional_db
from ...results.metrics import compute_downlink_summary_metrics
from ...simulation.system import DownlinkSystem
from .evaluate_precoder_network import evaluate_downlink_precoder_net
from .build_training_rollouts import build_training_dataset
from .train_precoder_network import (
    train_blocklength_aware_precoder_net,
)
from .precoder_network import build_precoder_net_artifact
...
def evaluate_trained_precoder_network_on_test_channel(
    ...
    *,
    ...
    """
    # TEST 1: Recreate one held-out joint channel and apply only test-time search overrides.
    ...
    # TEST 2: Construct the held-out BS system and its explicit scenario description.
    ...
    # TEST 3: Restore fixed trained weights from the artifact.
    ...
    # TEST 4: Run inference-only joint scheduling and blocklength allocation.
    ...
    # TEST 5: Attach training provenance, test metadata, metrics, and train/test cost split.
    ...
    # TEST OUTPUT: Generate held-out diagnostics from the completed result.
    if do_plots:
    ...
    # TEST OUTPUT: Persist the canonical result, summary, and scenario description.
...
def main() -> None:
    ...
    # DATA 4: Save reproducible training and held-out joint channel/block manifests.
    ...
    # TRAINING 1: Build rollout scenarios from the configured training channels.
    if artifact is None:
        ...
        # TRAINING 2: Optimize the persistent per-user nets or shared-BS net.
        # This is the backpropagation boundary; it returns trained models and epoch history.
        ...
        # TRAINING 3: Package model states, configuration, histories, and cost for reuse.
        ...
        # TRAINING OUTPUT: Persist the reusable weights and detailed training summaries.
    ...
    # TESTING: Evaluate the same fixed model artifact independently on every held-out seed.
    # Each helper call performs inference, n_kl search, metrics, plots, and result persistence.
