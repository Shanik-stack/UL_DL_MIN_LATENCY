"""Result assembly shared by downlink experiment methods."""
...
from ..configuration.loader import validate_shared_bs_streaming_blocklength_input_mode
from ..precoders.service import describe_precoder_parameterization
from ..precoders.checkpoints import export_user_model_specs


def build_monte_carlo_evaluation_result(
    *,
    system: Any,
    ...
    """Assemble a downlink Monte Carlo schedule into the common result schema."""
    ...
    result = {
        ...
        "precoder_parameterization": describe_precoder_parameterization(
            model_scope, "precoder_net", uses_blocklength_input=True
        ),
        ...
        "shared_bs_streaming_blocklength_input_mode": (
            validate_shared_bs_streaming_blocklength_input_mode(
                sim_params["shared_bs_streaming_blocklength_input_mode"]
            )
        ...
        "baseline_references": {
            "random_precoder_baseline": build_schedule_reference(
                "random_precoder_baseline", initial_latency, initial_plan,
                snr_db=initial_snr_db, sinr_db=initial_sinr_db,
            ),
            "naive_full_T_baseline": build_schedule_reference(
                "naive_full_T_baseline", full_block_latency, full_block_plan,
                snr_db=initial_snr_db, sinr_db=initial_sinr_db,
            ),
        },
    ...
    if scenario_block_targets is not None:
        result["scenario_block_targets"] = [
            list(map(int, row)) for row in scenario_block_targets
        ]
    return result


__all__ = ["build_monte_carlo_evaluation_result"]
